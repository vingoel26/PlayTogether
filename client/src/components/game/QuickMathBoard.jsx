 ...
 */
export default function QuickMathBoard({ gameState, onMove, onStart, onReset }) {
    ...
    <div style={{
      ...
      {isComplete ? (
        ...
        <div style={{
          ...
          <span className="material-symbols-outlined" style={{ fontSize: 56, color: '#F29900' }}>emoji_events</span>
          <h2 style={{ margin: 0, fontSize: 24 }}>{resultText}</h2>
          <div style={{ fontSize: 14, color: 'var(--color-text-on-dark-dim)' }}>
            Final Scores: {players.map(p => `${p.displayName}: ${scores[p.id] || 0}`).join(' • ')}
          </div>
          {isHost && (
            <button
              onClick={onReset}
